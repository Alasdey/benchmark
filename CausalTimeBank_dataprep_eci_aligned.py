#!/usr/bin/env python3
"""
CausalTimeBank — ECI (Event Causality Identification) dataprep
==============================================================
Variant of CausalTimeBank_dataprep_aligned.py restricted to the ECI task:

  • relations contains only "CLINK" — no TLINKs.
  ...
  • pair_list covers all annotated positive CLINKs by construction.
...
Output schema per row (identical fields to _aligned, narrower relations):
  ...
  relations  dict               {"CLINK": [[src_idx, tgt_idx], ...]}
...
def _parse_tml(path: str, doc_idx: int) -> Dict | None:
    ...
    # Intra-sentence CLINKs only (ECI task scope)
    clinks: List[List[int]] = []
    for clink in root.findall("CLINK"):
        ...
        if midx_to_sent.get(src_idx) == midx_to_sent.get(tgt_idx):
            clinks.append([src_idx, tgt_idx])

    relations: Dict[str, List[List[int]]] = {"CLINK": clinks}
...
def build_dataset(root_dir: str) -> Dataset:
    ...
    n_clinks = sum(len(r["relations"]["CLINK"]) for r in rows)
...
def parse_args():
    ...
    p.add_argument("--repo_id", default="Nofing/CausalTimeBank-causal", help="Target HF dataset repo, e.g. user/my-dataset.")
